...
static float clamped_scroll_delta_from_edge_distance(float distance_to_edge)
{
    ...
    return medit_clampf(delta, 0, MAX_DRAG_SCROLL_DELTA);
}
    ...
    DragState* drag_state)
{
    ...
    const bool dragging = drag_state->active_id != CLAY_EXT_NULL_ID && mb_left.state == MOUSE_BUTTON_PRESSED;
    ...
    if (scroll.config.horizontal) {
        ...
        const float min_x = -MEDIT_MAX(scroll.contentDimensions.width - scroll.scrollContainerDimensions.width, 0);
        scroll.scrollPosition->x = medit_clampf(scroll.scrollPosition->x, min_x, 0);
    }
    if (scroll.config.vertical) {
        scroll.scrollPosition->y += delta.y * config.sensitivity_v;
        const float min_y = -MEDIT_MAX(scroll.contentDimensions.height - scroll.scrollContainerDimensions.height, 0);
        scroll.scrollPosition->y = medit_clampf(scroll.scrollPosition->y, min_y, 0);
    }
}
